...
sub install
   ...
   io( $self->outfile )->print(<<"END_FILE_HEADER") if $self->outfile;
-- 
-- Posemo SQL File 
-- PostgreSQL Secure Monitoring
-- Generated by Posemo $PostgreSQL::SecureMonitoring::VERSION at $now
-- 
...
END_FILE_HEADER
   ...
   die "&extension not fully implemented yet!\n" if $self->extension;
...
sub do_sql
   ...
   if   ( $self->outfile ) { io( $self->outfile )->append( _end_query($sql) ); }
...
#
# Checks if the SQL ends with ; and newline
#
...
sub _end_query
   ...
   my $has_semicolon = $sql =~ m{ ; \s* \z }xs;
   my $has_newline   = $sql =~ m{ \n \s* \z }xs;

   return $sql if $has_newline and $has_semicolon;

   # insert ";" before all whitespaces
   $sql =~ s{ ( \s* ) \z}{;$1}xs unless $has_semicolon;

   # add newline at end unless there is it already
   $sql .= "\n" unless $has_newline;
...
=head2 around begin, commit, rollback, dbh

Modifies begin/commit from MooseX::DBI for writing BEGIN/COMMIT to a file (when outfile is set).
...
around begin => sub {
   ...
   if   ( $self->outfile ) { io( $self->outfile )->append("\nBEGIN;\n\n"); }
   else                    { $self->$orig; }
...
around rollback => sub {
   ...
   if ( $self->outfile ) { ERROR "ROLLBACK? Failure in install."; io( $self->outfile )->append("\nROLLBACK;\n\n"); }
   else                  { $self->$orig; }
...
#    my $self = shift;
#
#    return $self->$orig unless $self->outfile;
#    return $self;
#
# };
