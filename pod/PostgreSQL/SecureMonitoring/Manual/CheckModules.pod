...
instead you should override C<_build_install_sql>. Refer to the C<Writeable> check as an example.
...
In the config file, you can set the defined attributes, but all default attribtes too:
...
You can override all methods in there, but typically you may want to override or modify 
C<test_critical_warning>, C<execute> and C<_build_code>. C<execute> should only be modified 
