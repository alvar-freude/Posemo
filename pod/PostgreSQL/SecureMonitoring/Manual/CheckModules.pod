...
values for this database (or tabe, user, …) (which is in the above example only 
...
If your title is a table or name, use C<skip_table_re> or C<skip_user_re> or 
something similar depending on your title.


=item 3.

The first row should return the sum of all databases (tables, users, …). You can use a Common Table Expression (CTE, C<WITH>-Statement)
...
to this databases – see the configuration manual.
