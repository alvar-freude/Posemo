...
When there is a hard error, the result is undef.
...
should have this type, bit in C<multiline> the first column is a name (of a 
...
At the moment, there is no complete list oif result units. 
...
This is work in progress and there may be added new units or the existing 
may change etc.
...
The value of this key containt a arrayref with a list of the names for each column.

The names are directly taken from the SQL result and depend on the check. 
They may be used as name/description for each graph element in output.
...
check und it's result, and are about warning/critical thresholds or errors.

=over 4

=item C<warning_level>

Check configuration: a numerical value with the warning level. 
From config file or a default may be set by the check. 

=item C<critical_level>

Check configuration: a numerical value with the critical level. 
From config file or a default may be set by the check. 

=item C<lower_is_worse>

Check configuration. Hint, if a lower value is worse then a higher value. 

e.g. a lower cache hit ratio is worse then a higher one.
...
String with error message, when there is an fatal error (e.g. connection problem).
...
Usable as a hint for the displaying frontend, e.g., this contains a 0 and 100 for percent values.
